...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{fig/Tjunc0.png}
    \caption{自适应网格的T连接情形}
    \label{fig:freeTjunk}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{fig/unstable_case.png}
    \caption{负权重导致的不稳定现象}
    \label{fig:unstable}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/free_T.png}
    \caption{有约束T型连接示意图}
    \label{fig:free_T}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{fig/adaptive_transfer.png}
...
本节介绍DC-APIC积分器同自适应广义插值物质点法的结合步骤。根据基于DC-APIC粒子网格传输方式的物质点法，首先需要计算交界面处粒子法线和格点法线，接着根据相场梯度计算粒子与网格间的兼容性以指导后续粒子到网格(P2G)、网格到粒子(G2P)的物理量传输。接下来将分别介绍这三步和DC-APIC积分器的融合方法。
\subsection{自适应法线和兼容性计算}
与前文类似，首先通过固体粒子的负质量梯度场来确定固体的法向量。
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
其中 $m_i^s$ 是映射到一个网格节点的固体质量，$p^s$ 表示固体粒子，$\mathbf{n}_p^s$ 表示固体粒子的法向量，然后依次计算格点法线 $\mathbf{n}_i$ 和流体粒子法线$\mathbf{n}_p^f$：
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
与前文不同的是，这里的粒子与格点插值权重$w_{ip}$通过$C^0$连续的自适应网格形状函数积分得到：
\begin{equation}
    \begin{aligned}
        \hat{V}_p &= \int_{\Omega\cap\Omega_p} d\mathbf{x} \\
        w_{ip} &= \frac{1}{\hat{V}_p} \int_{\Omega} \chi_p(\mathbf{x}) N_i(\mathbf{x}) \, d\mathbf{x},
...
兼容性计算与前文也类似，唯一的区别是，通过粒子相位状态估计网格相位梯度时通过$\nabla s_i = \sum_{p} s_p \nabla w_{ip}$，此处的$ \nabla w_{ip} $应采用广义插值物质点法的定义：
\begin{equation}
    \nabla w_{ip} = \frac{1}{\hat{V}_p} \int_{\Omega_p} \nabla^{\mathbf{x}} N_i(\mathbf{x}) d\mathbf{x}.
\end{equation}

\subsection{自适应粒子网格传输}
融合DC-APIC积分器自适应粒子网格传输，在交界面梯度法线的指导下进行计算。对于粒子到网格传输阶段(G2P),粒子会首先找到当前粒子作用域$\Omega_p$内最细粒度的网格，在核函数半径内把质量传递到所有格点上（包括幽灵格点），将动量的切线方向传递到兼容的真是格点和幽灵格点上，然后进行递归地将幽灵节点的质量和动量传递给四叉树中的父节点，也就是更粗一级的网格，直到所有节点都是真实节点。
粒子到网格传输阶段(G2P)是反向的过程，物理量首先从最粗一级网格传递到最高分辨率网格上，再按照兼容性和边界法线的指导传输回相应粒子上。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{fig/adaptive_coll_3d.png}
