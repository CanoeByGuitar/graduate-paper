...
1. \textbf{粒子到网格传输（Particles to Grid, P2G）：}
这一步将粒子携带的质量、速度等物理量传输到背景网格中，传输方式有多种将在第~\ref{sec:trans}节中重点介绍，这里根据最常用的APIC传输方式进行介绍。首先粒子网格通过二次B样条基函数进行插值，其公式为：
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
其中，\(\mathbf{C}_p^n\)是表示粒子局部速度梯度矩阵，用于维持粒子的线性动量和角动量守恒。
...
3. \textbf{网格到粒子传输（Grid to Particles, G2P）：}
