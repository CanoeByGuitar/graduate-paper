\chapter{基于非连续速度场的流固耦合仿真} \label{chap:dcapic}
物质点法在进行物理仿真时存在的难以实现流固交界面的自由滑动边界条件，因为物质点法是一种粒子网格混合方法，粒子和网格需要来回传递质量、动量等物理量，如图~\ref{fig:p2g}所示，由于粒子网格传输依赖于B样条插值函数，相临粒子的速度不可避免地被平均化到网格格点上，因此插值出来的网格速度天然具有强连续性，另外由于物质点法的力学解算基于连续介质力学的本构模型，这种连续性被进一步加强。然而自然界的多材料耦合现象，往往伴随着分离现象，例如雨水从伞面滑落，因此实现基于非连续速度场的物质点法至关重要。

在第~\ref{sec:trans}节中阐述了各种粒子网格传输方式(积分器)的优点与不足，本章聚焦于改进物质点法中的积分器，创新地提出了分解兼容仿射粒子网格（DC-APIC）积分器算法，该算法有效地解决传统MPM算法中数值粘性问题。本章结构如下，首先提出粒子与网格兼容性的定义，并给出两种计算方式：基于符号距离函数场（SDF）的方法和基于相场梯度的方法；接着分辨阐述DC-APIC粒子到网格传输算法和网格到粒子传输算法，详细给出了质量和动量的传输公式，并论述了该方法维持动量守恒的特性；然后给出结合DC-APIC的物质点法的算法全流程，着重描述了在处理交接面几何边界时采用的不同法线计算方式；最后在多个二维和三维的实验中对比了本文同其他传统传输方式的优点，并罗列了时间开销。
...
对于每个粒子，由于不连续性约束的施加，不兼容网格节点上的切向速度与粒子无关。我们采用了幽灵粒子速度方法，在这种方法中，我们假设对于任意节点 $j\in i^{p-}$，其速度通过从粒子$p$的上一时间步速度常数外推得到，即 $\mathbf{v}_j=\mathbf{v}_p^n$。因此，从网格到粒子的DC-APIC传输方法，综合了来自兼容节点和不兼容节点的贡献，其表达式为：
\begin{equation}
    \label{eq:DCAPIC_g2p}
    \begin{aligned}
...
\end{aligned}
\end{equation}
其中 $\mathbf{n}_i$ 是网格节点的法向量，可以覆盖所有界面网格节点，$\mathbf{n}_p^f$ 表示流体粒子的法向量。从公式~\eqref{eq:ni_npf} 可以看出，我们首先使用 B 样条平滑核对固体粒子法向量 $\mathbf{n}_p^s$ 进行平滑，以得到网格节点的法向量。平滑核可以使 DC-APIC 的网格到粒子（第~\ref{sec:DC-APIC_p2g} 节）和粒子到网格（第~\ref{sec:DC-APIC_g2p} 节）更加稳定。然后，我们利用网格节点法向量来计算流体粒子的法向量。该策略既稳定又高效，避免了在计算流体法向量时需要通过空间数据结构查找最近的固体粒子。
...
\begin{table*}[htbp]
    ...
    \makebox[7em][l]{(Fig.~\ref{fig:sdf_phase_compare} top)} 
     二维球 & 0.78 & 0.05 & 0.001 & 8K & 4000 & 0.4 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:sdf_phase_compare} middle)} 
    二维球 & 0.42 & 0.05 & 0.001 & 8K & 4000 & 0.4 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:sdf_phase_compare} bottom)} 
    二维球 & 1.14 & 0.05 & 0.001 & 8K & 4000 & 0.4 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:bunny3d} left)} 钉住的兔子 & 6.33  & 0.1 & 0.001 & 98K & 8000 & 0.4  & 1.0\\
    \makebox[7em][l]{(Fig.~\ref{fig:bunny3d} right)} 钉住的兔子 & 3.60  & 0.1 & 0.001 & 98K & 8000 & 0.4  & 1.0\\
    \makebox[7em][l]{(Fig.~\ref{fig:duck})} 橡胶鸭子 & 57.63 & 0.1 & 0.001 & 640K & 6000 & 0.4 & 0.5/1.0/2.5 \\
    \makebox[7em][l]{(Fig.~\ref{fig:jelly} left)} 果冻和沙粒流 & 14.72 & 0.05 & $5 \times 10^{-4}$ & 1.2M & 1500 & 0.2 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:jelly} middle)} 果冻和沙粒流 & 17.16 & 0.05 & $5 \times 10^{-4}$ & 1.2M & 4000 & 0.3 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:jelly} right)} 果冻和沙粒流 & 15.91 & 0.05 & $5 \times 10^{-4}$ & 1.2M & 6000 & 0.4 & 1.0 \\
    \makebox[7em][l]{(Fig.~\ref{fig:bunny_bath})} 小熊洗澡 & 24.17 & 0.1 & $5 \times 10^{-4}$ & 400K & 6000 & 0.4 & 1.2 \\
    \hline
    \end{tabular}%
    }
...
\begin{figure}[htbp]
    ...
    \caption{二维实验：两种计算兼容性方法的DC-APIC算法和APIC算法的对比}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/duck.png}
    \caption{流固耦合固体密度测试}
    \label{fig:duck}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{figures/jelly_6.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/echarts.png}
...
\begin{figure}[htbp]
    ...
    \label{fig:asflip}
\end{figure}
图~\ref{fig:asflip}中展示了二维流固分离场景中APIC积分器（传统MPM）、ASFLIP积分器和本文提出的DC-APIC积分器的仿真效果的对比，ASFLIP不仅难以处理流固耦合边界的自然分离，还因为它不稳定的数值特性，导致部分流体粒子穿透固体表面，产生严重的数值稳定性；而DC-APIC积分器很好的实现了非连续速度场，使得物质点法能够实现流固耦合的自由滑移边界条件。
